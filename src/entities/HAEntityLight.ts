...
export class HAEntityLight extends HAEntityBoolean {
  ...
  setBrightnessRaw(value: number): HAEntityLight {
    if (value < 0 || value > 255) {
      console.warn('Trying to set brightness out of range 0 <= B <= 255. Value: ' + value);
  ...
  setBrightnessPercent(value: number): HAEntityLight {
    if (value < 0 || value > 100) {
      console.warn('Trying to set brightness percent out of range 0 <= B <= 100. Value: ' + value);
  ...
  setStateFromHA(state: IHAEntityState, skipSaveState: boolean = false) {
    ...
    this.supports.brightness = !!(this.attributes.supported_features & HALightSupportFeatureFlag.BRIGHTNESS);
    this.supports.colorTemp = !!(this.attributes.supported_features & HALightSupportFeatureFlag.COLOR_TEMP);
    this.supports.effect = !!(this.attributes.supported_features & HALightSupportFeatureFlag.EFFECT);
    this.supports.flash = !!(this.attributes.supported_features & HALightSupportFeatureFlag.FLASH);
    this.supports.color = !!(this.attributes.supported_features & HALightSupportFeatureFlag.COLOR);
    this.supports.transition = !!(this.attributes.supported_features & HALightSupportFeatureFlag.TRANSITION);
    this.supports.whiteValue = !!(this.attributes.supported_features & HALightSupportFeatureFlag.WHITE_VALUE);
  }
